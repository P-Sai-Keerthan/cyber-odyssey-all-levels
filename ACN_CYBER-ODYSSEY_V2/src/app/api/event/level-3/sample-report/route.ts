...
/**
 * Authenticated stream for Level 3 Sample Report / Reference Rubric Template.
...
export async function GET() {
  ...
  if (user.role === 'PARTICIPANT') {
    if (!user.membership) {
      return NextResponse.json(
        { error: 'Forbidden. Active squad membership is required to access Level 3 materials.' },
        { status: 403 },
      );
    }
    ...
    if (!access.allowed && !access.isCompleted) {
      return NextResponse.json(
        { error: access.reason || 'Level 3 reference materials are currently inaccessible.' },
        { status: 403 },
      );
    }
  }
  ...
  const resource = await getPublishedLevelResource(3, 'SAMPLE_REPORT');
  ...
  if (!resource || !resolvedPath || !fs.existsSync(resolvedPath)) {
    return NextResponse.json(
      {
        error:
          'The Level 3 sample report template has not been published yet. ' +
  ...
  await prisma.auditLog
    .create({
      data: {
        ...
        details: `User @${user.username} (${user.role}) downloaded Level 3 sample report ("${resource.originalName}").`,
      },
    })
    .catch(() => {});
